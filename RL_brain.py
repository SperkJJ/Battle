...
class PolicyGradient:
    def __init__(
        ...
        try:
            model_file = tf.train.latest_checkpoint('./new_logs')
    ...
    def make_cnn(inputs, scope_name):
        with tf.variable_scope(scope_name):
            ...
            conv_3 = tf.layers.conv2d(inputs=pool_1,filters=128,kernel_size=[3, 3],strides=1,padding="same",
            activation=tf.nn.leaky_relu,kernel_initializer=tf.uniform_unit_scaling_initializer())
            ...
            # dense layer II
            fc_2 = tf.layers.dense(inputs= dp_1, units=64, activation=tf.nn.leaky_relu, kernel_initializer=tf.uniform_unit_scaling_initializer())
    ...
    def deep_nn(input1, input2, input3, input4):
        ...
        fc = tf.layers.dense(inputs=out, units=9, kernel_initializer=tf.uniform_unit_scaling_initializer())
